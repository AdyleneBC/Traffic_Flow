...
// Inicia el tráfico automático
    ...
    IEnumerator SimularTraficoFluido()
    {
        ...
        while (true)
        {
            ...
            {
                MostrarEstado($"[Tráfico] Ruta inválida o bloqueada a {destino.id}");
            ...
            // Pequeña pausa antes de buscar nuevo destino
    ...
    IEnumerator MoverRutaFluida(List<Nodo> ruta)
    {
        for (int i = 1; i < ruta.Count; i++)
        {
            ...
            // Verifica que el camino esté abierto
            bool caminoAbierto = origen.conexiones.Exists(c => c.destino == destino && !c.cerrado);
            if (!caminoAbierto)
            {
                MostrarEstado($"[Tráfico] Camino cerrado hacia {destino.id}. Recalculando...");
                yield break;
            }

            MostrarEstado($"[Tráfico] → {destino.id}");
        ...
        MostrarEstado($"[Tráfico] Llegaste a {nodoActual.id}");
    }
